...
DefineInnerModel <- function() {
  LearnEmbed 	         <- c(0, 0, 0, 0, 0, 0, 0, 0)
	...
	SocialClimate 	     <- c(0, 0, 0, 0, 0, 0, 0, 0)
	IndividualMotivation <- c(0, 0, 0, 0, 0, 0, 0, 0)
	LearningCapability   <- c(1, 1, 0, 0, 0, 0, 0, 0)
	SocialNetwork	       <- c(0, 0, 0, 1, 1, 0, 0, 0)
	InnovationAbility    <- c(0, 0, 1, 0, 0, 1, 1, 0)
...
DefineOuterMode <- function(correctData) {
	...
	innovation_blocks <- list(
				  ...
				  GetIndex(colValues, Climate[c( 8, 12)]), 
				  GetIndex(colValues, Motivation[c(1,6)]), 
